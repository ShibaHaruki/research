...
def _cleanup_run_output_dir(run_out_dir: Path, *, reason: str) -> dict:
    """Delete one temporary liquid_run candidate directory after VAE pretraining use."""
    path = Path(run_out_dir)
    row = {
        "run_out_dir": str(path),
        "cleanup_reason": reason,
        "cleanup_target": "run_out_dir",
        "cleanup_status": "skipped",
        "cleanup_message": "",
    }
    if not str(path):
        row["cleanup_status"] = "missing"
        row["cleanup_message"] = "empty path"
        return row
    if not path.exists():
        row["cleanup_status"] = "missing"
    ...
    liquid_root = (RESULTS_PATH / "liquid_run").resolve()
    try:
        resolved.relative_to(liquid_root)
    except ValueError:
        row["cleanup_status"] = "blocked"
        row["cleanup_message"] = f"refuse to delete outside liquid_run root: {liquid_root}"
        return row
    try:
        ...
        return row
    if resolved == liquid_root:
        row["cleanup_status"] = "blocked"
        row["cleanup_message"] = "refuse to delete liquid_run root itself"
        return row

    try:
        shutil.rmtree(path)
        row["cleanup_status"] = "deleted"
        row["cleanup_message"] = "deleted temporary liquid_run candidate directory"
    ...
    *,
    ...
    def handle_candidate_result(result: dict) -> None:
        ...
        if exclude:
            ...
            if bool(cleanup_cfg.get("enabled", True)) and bool(cleanup_cfg.get("remove_excluded_immediately", True)):
                cleanup_rows.append(
                    {
                        **entry,
                        **_cleanup_run_output_dir(
                            Path(entry["run_out_dir"]),
                            reason="excluded_from_vae_pretrain",
                        ),
                    }
                )
        else:
        ...
        if cleanup_rows:
            pd.DataFrame(cleanup_rows).to_csv(out_dir / "pretrain_liquid_run_cleanup.csv", index=False)
    ...
    vae_cfg.update(dict(pretrain_cfg.get("vae", {})))
    if "vae_progress_interval" in pretrain_cfg:
        vae_cfg["progress_interval"] = pretrain_cfg["vae_progress_interval"]
    ...
    if bool(cleanup_cfg.get("enabled", True)) and bool(cleanup_cfg.get("remove_used_after_training", True)):
        remove_liquid_run = bool(cleanup_cfg.get("remove_liquid_run_after_training", True))
        for entry in entries:
            if remove_liquid_run:
                cleanup_row = {
                    **entry,
                    **_cleanup_run_output_dir(
                        Path(entry["run_out_dir"]),
    ...
    print(f"[fixed-vae] training_loss_csv={vae_result.get('history_file', '')}")
    print(f"[fixed-vae] training_loss_plot={vae_result.get('history_plot_file', '')}")
