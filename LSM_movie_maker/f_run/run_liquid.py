...
from d_tools.visualization import (
    safe_stem,
    save_gif_sequence,
    save_lsm_schematic_activity_movie,
    save_side_by_side_gif,
    save_liquid_3d_plot,
    save_liquid_network_activity_movie,
    save_liquid_process_movie,
    save_spike_raster,
    save_voltage_plot,
...
def make_liquid_network(net_cfg: dict, N_in: int, rng: np.random.Generator, run_cfg: dict):
    ...
    fixed_out_cfg = dict(net_cfg)
    fixed_out_cfg["learning_rule"] = "off"
    S_lo, _ = make_liq_to_out_synapses(G_liq, G_out, rng, fixed_out_cfg)
    ...
    internal_state_dt_ms = min(
        float(run_cfg.get("INTERNAL_STATE_DT_MS", 1.0)),
        voltage_dt_ms,
    )
    V_liq_internal = [
        StateMonitor(
    ...
    for monitor in M_liq_debug + M_out_debug + V_liq + V_liq_internal + V_out:
    ...
    net.add(*(M_liq_debug + M_out_debug + V_liq + V_liq_internal + V_out))
...
def liquid_weight_groups(objects: dict) -> dict[str, list]:
    return {
        ...
        "liquid_to_output_fixed": objects.get("S_lo", []),
    }
...
def clear_monitor_history(monitors) -> None:
...
def clock_time_copy():
    return float(defaultclock.t / ms) * ms
...
def save_first_sample_debug(
    ...
    debug_full_voltage_start_indices: list[int] | None = None,
    debug_output_spike_start_indices: list[int] | None = None,
    debug_output_voltage_start_indices: list[int] | None = None,
    ...
    process_movie_fp = debug_dir / "process_movie" / f"{tag}_process.gif"
    schematic_movie_fp = debug_dir / "process_movie" / f"{tag}_lsm_schematic.gif"
    save_liquid_process_movie(
        process_movie_fp,
        mat=mat,
        sid=sid,
        ...
        output_spike_monitors=objects["M_out_debug"],
        output_voltage_monitors=objects["V_out"],
        output_voltage_indices=objects["V_out_indices"],
        duration_ms=duration_ms,
        spike_start_indices=debug_spike_start_indices,
        voltage_start_indices=debug_voltage_start_indices,
        output_spike_start_indices=debug_output_spike_start_indices,
        output_voltage_start_indices=debug_output_voltage_start_indices,
        start_time=debug_start_time,
        fps=float(RUN_CFG.get("PROCESS_MOVIE_FPS", 12.0)),
        seconds=float(RUN_CFG.get("PROCESS_MOVIE_SECONDS", 8.0)),
        select_voltage_by_spike_count=False,
    )
    if objects["G_liq"] and objects["M_liq_debug"]:
        save_liquid_network_activity_movie(
            debug_dir / "process_movie" / f"{tag}_lsm2d.gif",
            group=objects["G_liq"][0],
            synapses=objects["S_intra"],
            spike_monitor=objects["M_liq_debug"][0],
            output_group=objects["G_out"][0] if objects.get("G_out") else None,
            output_synapses=objects.get("S_lo"),
            output_spike_monitor=(
                objects["M_out_debug"][0] if objects.get("M_out_debug") else None
            ),
            title=f"{mat} sid{sid} LSM 2D activity",
            duration_ms=duration_ms,
            spike_start_index=(
                debug_spike_start_indices[0]
                if debug_spike_start_indices
                else None
            ),
            output_spike_start_index=(
                debug_output_spike_start_indices[0]
                if debug_output_spike_start_indices
                else None
            ),
            start_time=debug_start_time,
            fps=float(RUN_CFG.get("PROCESS_MOVIE_FPS", 12.0)),
            seconds=float(RUN_CFG.get("PROCESS_MOVIE_SECONDS", 8.0)),
            max_edges=int(RUN_CFG.get("PROCESS_LSM2D_MAX_EDGES", 1500)),
            spike_window_ms=float(RUN_CFG.get("PROCESS_LSM2D_SPIKE_WINDOW_MS", 5.0)),
        )
    if objects["G_liq"] and objects.get("G_out") and objects["M_liq_debug"] and objects.get("M_out_debug"):
        save_lsm_schematic_activity_movie(
            schematic_movie_fp,
            mat=mat,
            sid=sid,
            input_current=input_current,
            t_array=t_array,
            liquid_group=objects["G_liq"][0],
            output_group=objects["G_out"][0],
            liquid_spike_monitor=objects["M_liq_debug"][0],
            output_spike_monitor=objects["M_out_debug"][0],
            input_to_liquid_synapses=objects["S_in"],
            liquid_to_output_synapses=objects["S_lo"],
            duration_ms=duration_ms,
            liquid_spike_start_index=(
                debug_spike_start_indices[0]
                if debug_spike_start_indices
                else None
            ),
            ...
            fps=float(RUN_CFG.get("PROCESS_MOVIE_FPS", 12.0)),
            seconds=float(RUN_CFG.get("PROCESS_MOVIE_SECONDS", 8.0)),
            max_edges=int(RUN_CFG.get("PROCESS_SCHEMATIC_MAX_EDGES", 18)),
            spike_window_ms=float(RUN_CFG.get("PROCESS_SCHEMATIC_SPIKE_WINDOW_MS", 8.0)),
        )
        save_side_by_side_gif(
            debug_dir / "process_movie" / f"{tag}_combined.gif",
            schematic_movie_fp,
            process_movie_fp,
            gap_px=int(RUN_CFG.get("PROCESS_COMBINED_GAP_PX", 4)),
            max_width_px=int(RUN_CFG.get("PROCESS_COMBINED_MAX_WIDTH_PX", 1600)),
            max_height_px=int(RUN_CFG.get("PROCESS_COMBINED_MAX_HEIGHT_PX", 900)),
            crop_padding_px=int(RUN_CFG.get("PROCESS_COMBINED_CROP_PADDING_PX", 6)),
        )
    ...
    output_voltage_starts = debug_output_voltage_start_indices or []
    for layer_index, monitor in enumerate(objects["V_out"]):
        save_voltage_plot(
            ...
            f"{mat} sid{sid} output L{layer_index + 1} membrane voltage",
            start_index=(
...
def run_liquid(cfg: dict | int | None = None, legacy_cfg: dict | None = None):
    ...
    combined_movie_paths: list[Path] = []
    ...
    try:
        for mat_index, mat in enumerate(materials):
            ...
            for sample_index in iterator:
                ...
                for monitor in objects["M_out_debug"]:
                    monitor.active = debug_active
                for monitor in objects["V_liq"]:
                    monitor.active = debug_active
                for monitor in objects["V_liq_internal"]:
                    monitor.active = debug_active or (
                        internal_state_active and internal_state_uses_voltage
                    )
                for monitor in objects["V_out"]:
                    monitor.active = debug_active
                if debug_active:
                    clear_monitor_history(objects["M_liq_debug"])
                    clear_monitor_history(objects["M_out_debug"])
                    clear_monitor_history(objects["V_liq"])
                    clear_monitor_history(objects["V_liq_internal"])
                    clear_monitor_history(objects["V_out"])
                sample_start_time = clock_time_copy()
                ...
                debug_output_spike_start_indices = None
                if debug_active:
                    debug_output_spike_start_indices = [
                        len(monitor.t) for monitor in objects["M_out_debug"]
                    ]
                debug_output_voltage_start_indices = None
                if debug_active:
                    debug_output_voltage_start_indices = [
                        len(monitor.t) for monitor in objects["V_out"]
                    ]
                debug_full_voltage_start_indices = None
                if debug_active:
                    debug_full_voltage_start_indices = [
                        len(monitor.t) for monitor in objects["V_liq_internal"]
                    ]
                ...
                if debug_active:
                    ...
                    future = save_first_sample_debug(
                        ...
                        debug_full_voltage_start_indices=debug_full_voltage_start_indices,
                        debug_output_spike_start_indices=debug_output_spike_start_indices,
                        debug_output_voltage_start_indices=debug_output_voltage_start_indices,
                    ...
                    combined_movie_fp = (
                        material_debug_dir
                        / "process_movie"
                        / f"{safe_stem(mat)}_sid{sid}_combined.gif"
                    )
                    if combined_movie_fp.exists():
                        combined_movie_paths.append(combined_movie_fp)
    ...
    if combined_movie_paths:
        all_movie_fp = debug_dir / "process_movie" / "all_materials_combined.gif"
        save_gif_sequence(
            all_movie_fp,
            combined_movie_paths,
            max_width_px=int(run_cfg.get("PROCESS_COMBINED_MAX_WIDTH_PX", 1600)),
            max_height_px=int(run_cfg.get("PROCESS_COMBINED_MAX_HEIGHT_PX", 900)),
        )
        print(f"[movie] saved all-material combined GIF to {all_movie_fp}")
