...
def _save_explained_variance(out_dir: Path, pca_result: dict) -> tuple[Path, Path]:
    ...
    for component_index, ratio in enumerate(ratios, start=1):
        plt.text(component_index, ratio, f"{ratio * 100:.1f}%", ha="center", va="bottom", fontsize=9)
...
def _pc_label(component_index: int, ratios: np.ndarray) -> str:
    label = f"PC{component_index + 1}"
    if component_index < len(ratios):
        label += f" ({ratios[component_index] * 100:.1f}%)"
    return label
...
def _scatter_scores_2d(
    ...
    plt.xlabel(_pc_label(x_index, explained_ratios), fontsize=14)
    plt.ylabel(_pc_label(y_index, explained_ratios), fontsize=14)
...
def _scatter_scores_3d(
    ...
    ax.set_xlabel(_pc_label(0, explained_ratios))
    ax.set_ylabel(_pc_label(1, explained_ratios))
    ax.set_zlabel(_pc_label(2, explained_ratios))
...
def _save_score_plots(out_dir: Path, pca_result: dict, labels: Sequence[str]) -> list[Path]:
    ...
    out_dir = Path(out_dir)
    explained_ratios = np.asarray(pca_result["explained_variance_ratio"], dtype=np.float64)
    plot_fps = [
        _scatter_scores_2d(
            plt,
            scores,
...
def _save_component_heatmaps(
    out_dir: Path,
    pca_result: dict,
    *,
    max_components: int = 20,
) -> list[Path]:
    n_neurons = pca_result.get("state_neurons")
    n_time = pca_result.get("state_time_steps")
    if n_neurons is None or n_time is None:
        return []

    components = np.asarray(pca_result["components"], dtype=np.float64)
    singular_values = np.asarray(pca_result["singular_values"], dtype=np.float64)
    ratios = np.asarray(pca_result["explained_variance_ratio"], dtype=np.float64)
    ...
    heatmap_dir = Path(out_dir) / "pca_component_heatmaps"
    heatmap_dir.mkdir(parents=True, exist_ok=True)

    n_plot = min(int(max_components), components.shape[0])
    plot_fps: list[Path] = []
    component_maps = [
        components[component_index].reshape(n_neurons, n_time)
        for component_index in range(n_plot)
    ]
    vmax = max(float(np.percentile(np.abs(item), 99.0)) for item in component_maps)
    vmax = vmax if vmax > 0 else 1.0

    for component_index, component_map in enumerate(component_maps):
        pc = component_index + 1
        singular = singular_values[component_index] if component_index < len(singular_values) else np.nan
        ratio = ratios[component_index] if component_index < len(ratios) else np.nan
        out_fp = heatmap_dir / f"pca_component_pc{pc:02d}_heatmap.png"
        ...
        plt.title(f"PC{pc} loading heatmap | singular={singular:.4g} | explained={ratio * 100:.1f}%")
        plt.tight_layout()
        plt.savefig(out_fp, dpi=150)
        plt.close()
        plot_fps.append(out_fp)

        neuron_mean = np.mean(component_map, axis=1)
        order = np.argsort(neuron_mean)[::-1]
        sorted_map = component_map[order, :]
        sorted_mean = neuron_mean[order]
        positive_count = int(np.count_nonzero(sorted_mean > 0))
        negative_count = int(np.count_nonzero(sorted_mean < 0))

        group_fp = heatmap_dir / f"pca_component_pc{pc:02d}_neuron_groups.png"
        ...
        if 0 < positive_count < sorted_map.shape[0]:
            ax.axhline(positive_count - 0.5, color="black", linestyle="--", linewidth=1.0)
        ax.set_xlabel("Time bin")
        ax.set_ylabel("Neurons sorted by time-mean loading")
        ax.set_title(
            f"PC{pc} grouped loading heatmap | +={positive_count}, -={negative_count} | "
            f"singular={singular:.4g} | explained={ratio * 100:.1f}%"
        )
        fig.colorbar(im, ax=ax, label="Component loading")
        fig.tight_layout()
        fig.savefig(group_fp, dpi=150)
        plt.close(fig)
        plot_fps.append(group_fp)
    ...
    for axis in axes.ravel():
        axis.axis("off")
    for component_index, component_map in enumerate(component_maps):
        axis = axes[component_index // n_cols][component_index % n_cols]
        pc = component_index + 1
        singular = singular_values[component_index] if component_index < len(singular_values) else np.nan
        ratio = ratios[component_index] if component_index < len(ratios) else np.nan
        axis.imshow(
        ...
        axis.set_title(f"PC{pc} s={singular:.3g}, {ratio * 100:.1f}%", fontsize=10)
        axis.set_xlabel("time bin", fontsize=8)
        axis.set_ylabel("neuron", fontsize=8)
        axis.tick_params(labelsize=7)
        axis.axis("on")
    grid_fp = heatmap_dir / f"pca_components_pc1_pc{n_plot}_heatmap_grid.png"
    fig.tight_layout()
    fig.savefig(grid_fp, dpi=150)
    plt.close(fig)
    plot_fps.append(grid_fp)
    return plot_fps


def _save_neuron_component_contributions(
    out_dir: Path,
    pca_result: dict,
    *,
    top_k: int = 20,
    ...
    contribution_dir = Path(out_dir) / "pca_neuron_contributions"
    ...
    for component_index, component in enumerate(components):
        component_map = component.reshape(n_neurons, n_time)
        contribution = np.sum(component_map * component_map, axis=1)
        total = float(np.sum(contribution))
        if total > 0:
            contribution_ratio = contribution / total
        else:
            contribution_ratio = np.zeros_like(contribution)

        signed_mean = np.mean(component_map, axis=1)
        signed_sum = np.sum(component_map, axis=1)
        abs_sum = np.sum(np.abs(component_map), axis=1)
        max_abs = np.max(np.abs(component_map), axis=1)
        peak_time_bin = np.argmax(np.abs(component_map), axis=1)
        order = np.argsort(contribution_ratio)[::-1]
        top_sets[component_index] = set(int(i) for i in order[:top_k])
        contribution_vectors.append(contribution_ratio)

        cumulative = 0.0
        ranks = np.empty(n_neurons, dtype=np.int32)
        ranks[order] = np.arange(1, n_neurons + 1, dtype=np.int32)
        cumulative_by_neuron = np.empty(n_neurons, dtype=np.float64)
        for neuron_index in order:
            cumulative += float(contribution_ratio[neuron_index])
            cumulative_by_neuron[neuron_index] = cumulative

        pc = component_index + 1
        explained = ratios[component_index] if component_index < len(ratios) else np.nan
            ...
            rows.append(
                {
                    ...
                    "rank_in_component": int(ranks[neuron_index]),
                    "contribution_ratio": float(contribution_ratio[neuron_index]),
                    "cumulative_contribution_ratio_by_rank": float(
                        cumulative_by_neuron[neuron_index]
                    ),
                    "signed_mean_loading": float(signed_mean[neuron_index]),
                    "signed_sum_loading": float(signed_sum[neuron_index]),
                    "abs_sum_loading": float(abs_sum[neuron_index]),
                    "max_abs_loading": float(max_abs[neuron_index]),
                    "peak_time_bin": int(peak_time_bin[neuron_index]),
                }
            )

    neuron_csv_fp = contribution_dir / "pca_neuron_component_contributions.csv"
    pd.DataFrame(rows).to_csv(neuron_csv_fp, index=False)

    top_rows = [
        row
        for row in rows
        if int(row["rank_in_component"]) <= top_k
    ]
    top_csv_fp = contribution_dir / f"pca_top{top_k}_neurons_by_component.csv"
    pd.DataFrame(top_rows).sort_values(
        ["component_index", "rank_in_component"]
    ).to_csv(top_csv_fp, index=False)

    overlap_rows = []
    n_components = len(contribution_vectors)
    for left_index in range(n_components):
        left = contribution_vectors[left_index]
        left_norm = float(np.linalg.norm(left))
        for right_index in range(left_index + 1, n_components):
            right = contribution_vectors[right_index]
            right_norm = float(np.linalg.norm(right))
            denominator = left_norm * float(right_norm)
            cosine = float(np.dot(left, right) / denominator) if denominator > 0 else 0.0
            union = top_sets[left_index] | top_sets[right_index]
            intersection = top_sets[left_index] & top_sets[right_index]
            jaccard = float(len(intersection) / len(union)) if union else 0.0
            overlap_rows.append(
                {
                    "component_a": f"PC{left_index + 1}",
                    "component_b": f"PC{right_index + 1}",
                    "top_k": int(top_k),
                    "top_k_overlap_count": int(len(intersection)),
                    "top_k_jaccard": jaccard,
                    "contribution_cosine_similarity": cosine,
                }
            )

    overlap_csv_fp = contribution_dir / "pca_neuron_contribution_overlap.csv"
    pd.DataFrame(overlap_rows).to_csv(overlap_csv_fp, index=False)
    ...
    if plt is not None:
        ...
        for component_index in range(n_components):
            pc = component_index + 1
            df_pc = top_df[top_df["component_index"] == pc].sort_values(
                "rank_in_component"
            )
            if df_pc.empty:
                continue
            out_fp = contribution_dir / f"pca_pc{pc:02d}_top{top_k}_neuron_contributions.png"
            fig, ax = plt.subplots(figsize=(9, 4.8))
            ax.bar(
                np.arange(len(df_pc)),
                df_pc["contribution_ratio"].to_numpy(dtype=np.float64),
                color="tab:blue",
                alpha=0.8,
            )
            ax.set_xticks(np.arange(len(df_pc)))
            ax.set_xticklabels(
                [str(int(v)) for v in df_pc["neuron_index"].to_list()],
                rotation=45,
                ha="right",
            )
            ax.set_xlabel("Neuron index")
            ax.set_ylabel("Contribution ratio")
            ax.set_title(f"PC{pc} top {top_k} neuron contributions")
        ...
        if n_components >= 2:
            matrix = np.eye(n_components, dtype=np.float64)
            for row in overlap_rows:
                i = int(str(row["component_a"]).replace("PC", "")) - 1
                j = int(str(row["component_b"]).replace("PC", "")) - 1
                matrix[i, j] = matrix[j, i] = float(row["contribution_cosine_similarity"])
            out_fp = contribution_dir / "pca_neuron_contribution_cosine_similarity.png"
            ...
            ax.set_title("Neuron contribution overlap")
    ...
    return {
        "neuron_contributions_csv": str(neuron_csv_fp),
        "top_neurons_csv": str(top_csv_fp),
        "overlap_csv": str(overlap_csv_fp),
        "plots": [str(path) for path in plot_fps],
        "top_k": int(top_k),
    }


def _save_material_pc_score_heatmap(
    out_dir: Path,
    pca_result: dict,
    labels: Sequence[str],
    ...
    mean_scores = np.zeros((len(unique_labels), scores.shape[1]), dtype=np.float64)
    for label_index, label in enumerate(unique_labels):
        mask = np.asarray([str(item) == label for item in labels], dtype=bool)
        if np.any(mask):
            mean_scores[label_index, :] = np.mean(scores[mask], axis=0)

    vmax = float(np.max(np.abs(mean_scores))) if mean_scores.size else 0.0
    vmax = vmax if vmax > 0 else 1.0

    out_fp = Path(out_dir) / "pca_material_pc_mean_scores_heatmap.png"
    plt.figure(figsize=(max(8, 0.45 * scores.shape[1]), max(4, 0.35 * len(unique_labels))))
    plt.imshow(mean_scores, aspect="auto", cmap="RdBu_r", vmin=-vmax, vmax=vmax)
    plt.colorbar(label="Mean PCA score")
    plt.xticks(
        np.arange(scores.shape[1]),
        [f"PC{i + 1}" for i in range(scores.shape[1])],
        rotation=45,
        ha="right",
    )
    ...
    plt.xlabel("Principal component")
    plt.ylabel("Material")
    plt.title("Material mean PCA scores")
    plt.tight_layout()
    plt.savefig(out_fp, dpi=150)
    plt.close()

    pc_denominator = np.max(np.abs(mean_scores), axis=0)
    pc_denominator = np.where(pc_denominator > 0, pc_denominator, 1.0)
    normalized_scores = mean_scores / pc_denominator

    normalized_fp = Path(out_dir) / "pca_material_pc_mean_scores_per_pc_normalized_heatmap.png"
    plt.figure(figsize=(max(8, 0.45 * scores.shape[1]), max(4, 0.35 * len(unique_labels))))
    plt.imshow(normalized_scores, aspect="auto", cmap="RdBu_r", vmin=-1.0, vmax=1.0)
    plt.colorbar(label="Mean PCA score / max abs score in PC")
    plt.xticks(
        np.arange(scores.shape[1]),
        [f"PC{i + 1}" for i in range(scores.shape[1])],
        rotation=45,
    ...
    plt.xlabel("Principal component")
    plt.ylabel("Material")
    plt.title("Material mean PCA scores (per-PC normalized)")
    plt.tight_layout()
    plt.savefig(normalized_fp, dpi=150)
    plt.close()
    return out_fp, normalized_fp
...
def save_internal_state_pca(
    ...
    *,
    ...
    component_heatmap_fps = _save_component_heatmaps(out_dir, pca_result, max_components=20)
    neuron_contribution_summary = _save_neuron_component_contributions(
        out_dir,
        pca_result,
        top_k=20,
    )
    material_pc_heatmap_fp, material_pc_normalized_heatmap_fp = _save_material_pc_score_heatmap(
        out_dir,
        pca_result,
        row_labels,
    )
    ...
    summary = {
        ...
        "component_heatmaps": [str(path) for path in component_heatmap_fps],
        "neuron_contributions": neuron_contribution_summary,
        "material_pc_score_heatmap": (
            str(material_pc_heatmap_fp) if material_pc_heatmap_fp is not None else ""
        ),
        "material_pc_score_per_pc_normalized_heatmap": (
            str(material_pc_normalized_heatmap_fp)
            if material_pc_normalized_heatmap_fp is not None
            else ""
        ),
