"""Run the liquid layer and save internal states for each material."""
...
prefs.codegen.target = str(RUN_CFG.get("BRIAN_CODEGEN_TARGET", "numpy"))
for log_name in RUN_CFG.get("BRIAN_SUPPRESS_LOG_NAMES", []):
    BrianLogger.suppress_name(str(log_name))
...
def reset_liquid_state(objects: dict, net_cfg: dict) -> None:
    reset_dynamic_state({"G_liq": objects["G_liq"], "S_all": objects["S_all"]}, net_cfg)
...
def save_input_plots(
    ...
    # 使用した入力フィルタごとに入力電流の波形を debug/<素材名> に保存する。
    channels, filters = _input_layout(input_filter_map)
    input_rows = _input_rows(input_filter_map)
    filters = list(dict.fromkeys(filter_name for _, filter_name in input_rows))
    t_ms = t_array * 1000.0

    for filter_name in filters:
        plt.figure(figsize=(10, 4))
        plotted_channels = []
        for row_index, (channel, row_filter_name) in enumerate(input_rows):
            if row_filter_name != filter_name:
                continue
...
def _spike_times_ms(
    ...
    *,
    ...
    if t_ms.size:
        t_ms = t_ms[mask]
    return t_ms[(t_ms >= 0.0) & (t_ms < float(duration_ms))]


def save_spike_count_plot(
    debug_dir: Path,
    tag: str,
    mat: str,
    sid: int,
    ...
    *,
    ...
    spike_starts = spike_start_indices or []
    layer_rates = []
    total_counts = np.zeros(edges.size - 1, dtype=float)

    for layer_index, monitor in enumerate(objects["M_liq_debug"]):
        start_index = spike_starts[layer_index] if layer_index < len(spike_starts) else None
        t_ms = _spike_times_ms(
            monitor,
            start_index=start_index,
            start_time=start_time,
            duration_ms=duration_ms,
        )
        counts, _ = np.histogram(t_ms, bins=edges)
        layer_rates.append(counts.astype(float))
        total_counts += counts

    plt.figure(figsize=(10, 4))
    plt.plot(centers, total_counts, color="black", linewidth=2.0, label="all layers")
    for layer_index, counts in enumerate(layer_rates):
        plt.plot(centers, counts, linewidth=1.2, alpha=0.8, label=f"L{layer_index + 1}")
    plt.xlabel("Time [ms]")
    plt.ylabel(f"Spike count / {bin_ms:g} ms")
    total_spikes = int(np.sum(total_counts))
    plt.title(f"{mat} sid{sid} spike count | bin={bin_ms:g} ms | spikes={total_spikes}")
    plt.xlim(0.0, duration_ms)
    max_count = max(
        [float(np.max(total_counts)) if total_counts.size else 0.0]
        + [float(np.max(counts)) if counts.size else 0.0 for counts in layer_rates]
    )
    plt.ylim(0.0, max(1.0, max_count * 1.1))
    if total_spikes == 0:
        plt.text(
            0.5,
            0.5,
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(debug_dir / f"{tag}_spike_count_{int(round(bin_ms))}ms.png", dpi=150)
    plt.close()
...
def save_first_sample_debug(
    ...
    save_spike_count_plot(
        ...
        start_time=debug_start_time,
        bin_ms=float((run_cfg or {}).get("DEBUG_FIRING_RATE_BIN_MS", 25.0)),
    )
...
def save_sample_internal_state(
    ...
    *,
    ...
    # 1試行分の内部状態を保存する。source に応じて電位またはスパイク由来の状態を使う。
...
def run_liquid(cfg: dict | int | None = None, legacy_cfg: dict | None = None):
    # リキッド単体実行の本体。学習せず、素材ごとに指定サンプル数の内部状態を保存する。
    ...
    sensor_gain = cfg.get("sensor_gain", {})
    filter_gain = cfg.get("filter_gain", {})
    opt_filter_gain = cfg.get("opt_filter_gain", {})
    ...
    try:
        for mat_index, mat in enumerate(materials):
            ...
            for sample_index in iterator:
                ...
                # debug は素材ごとの最初の1試行だけ。内部状態保存は全サンプルで行う。
                ...
                if internal_state_active:
                    # VAE/PCA/分類などで使う内部状態を material ごとのフォルダへ保存する。
                    material_internal_state_dir = internal_state_dir / safe_stem(mat)
                    material_internal_state_dir.mkdir(parents=True, exist_ok=True)
    ...
    save_weight_distributions(
        # 実行後の重みも保存する。run_liquid では学習しないため、通常は初期値と同じ。
        debug_dir,
        liquid_weight_groups(objects),
        tag="final",
    )
