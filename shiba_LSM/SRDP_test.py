...
# ============================================================
# SRDP_parallel_TEST_multi_liquid_L1to4_total1000.py
#
# 目的:
#  - SRDP学習後に保存した重み（w_in, w_res, w_out）を読み込み
...
#  - SRDP_parallel_L{L}_N{ntag}_liq{k}_w_in.npy
#    SRDP_parallel_L{L}_N{ntag}_liq{k}_w_res.npy
#    SRDP_parallel_L{L}_N{ntag}_liq{k}_w_out.npy   (dense)
#
# 出力:
#  - SRDP_parallel_TEST_L{L}_N{ntag}_sout_rec.npy
#
# 追加:
#  - 進捗表示（全体バー、run中バー(任意)）
...
# --- 重要: ここは numpy import より前に書く（並列時の過剰スレッド抑制） ---
import os
...
MAX_WORKERS = None        # NoneならCPUコア数に合わせる（最大4）
...
# 重みロード（SRDP prefix）
# ============================================================
def load_trained_weights_srdp(L: int, n_res_list: list[int]):
    ...
    # 実行ディレクトリに依存しないよう、スクリプトと同じ場所から読む
    weights_dir = Path(__file__).resolve().parent

    for k, n_res in enumerate(n_res_list, start=1):
        f_in  = weights_dir / f"SRDP_parallel_L{L}_N{ntag}_liq{k}_w_in.npy"
        f_res = weights_dir / f"SRDP_parallel_L{L}_N{ntag}_liq{k}_w_res.npy"
        f_out = weights_dir / f"SRDP_parallel_L{L}_N{ntag}_liq{k}_w_out.npy"
...
# テスト本体（並列リキッド）
# ============================================================
def run_test_srdp_parallel_liquids(L: int, n_res_list: list[int]):
    ntag, N_in, N_out, W_in_list, W_res_list, W_out_list = load_trained_weights_srdp(L, n_res_list)
    ...
    t0 = 0 * ms  # サンプル開始時刻として使う（サンプル終わりに更新）
    ...
    # ----------------------------
    # test loop（進捗: 全体=素材×サンプル）
    # ----------------------------
    total_iters = len(dir_name) * N_SAMPLE_PER_MAT
    with tqdm(total=total_iters, desc=f"SRDP-TEST L{L} N[{ntag}]", dynamic_ncols=True) as pbar:
    ...
    out_name = f"SRDP_parallel_TEST_L{L}_N{ntag}_sout_rec.npy"
    np.save(out_name, sout_rec)
    print("[SAVED]", out_name, sout_rec.shape)

# ============================================================
# main（Lごとに並列実行）
# ============================================================
def _run_one_L(job):
    """ProcessPoolExecutor から呼ぶためのラッパ（pickle安定化用）"""
    L, n_res_list = job
    run_test_srdp_parallel_liquids(L, n_res_list)
    return L
...
if __name__ == "__main__":
    ...
    if not PARALLEL_RUN:
        for L in tqdm(LIQUID_COUNTS, desc="L loop", dynamic_ncols=True):
            run_test_srdp_parallel_liquids(L, N_RES_BY_L[L])
        ...
        ctx = mp.get_context("spawn")  # Windowsではspawn推奨
