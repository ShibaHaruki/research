# ============================================================
# STDP_parallel_multi_liquid_L1to4_total1000_clipW_histONLY.py
# （Brian2 の予約語 _pre/_post 回避版）
# ============================================================
...
# ============================================================
# STDP（ペア）パラメータ
#   ※Brian2予約語回避のため「tau_pre/tau_post」は使わない
# ============================================================
wmin = W_MIN
wmax = W_MAX

tau_pre_val  = 11.7 * ms
tau_post_val = 14.0 * ms

A_plus_val   = 0.0007
A_minus_val  = -0.0006   # LTD用（負）
...
# ----------------------------
# STDP（ペア）
# 予約語回避: tau_pre_stdp / tau_post_stdp を使う
# ----------------------------
STDP = """
...
tau_pre_stdp  : second (constant)
tau_post_stdp : second (constant)
...
# ============================================================
def run_stdp_parallel_liquids(n_res_list: list[int]):
    L = len(n_res_list)
    ...
    for k, n_res in enumerate(n_res_list):
        ...
        # output synapses (STDP sparse)
        S_out = Synapses(
            G_res, G_out,
            model=STDP,
            on_pre=on_pre_out + STDP_pre,
            on_post=STDP_post,
            method="euler",
            namespace=ns,
        )
        ...
        # constants on synapses（★名前変更した所）
        S_out.tau_pre_stdp  = tau_pre_val
        S_out.tau_post_stdp = tau_post_val
        S_out.A_plus  = A_plus_val
        S_out.A_minus = A_minus_val
    ...
    for epo in range(EPOCHS):
        for i_size in range(I_SIZE_RANGE):
            for material in tqdm(dir_name, desc=f"STDP parallel L{L} N[{ntag}] epo={epo} i_size={i_size}"):
    ...
    # save per liquid
    for k, n_res in enumerate(n_res_list):
        np.save(f"STDP_parallel_L{L}_N{ntag}_liq{k+1}_w_in.npy",  clipW(W_in_list[k]))
        np.save(f"STDP_parallel_L{L}_N{ntag}_liq{k+1}_w_res.npy", clipW(W_res_dense_list[k]))
    ...
    for k, n_res in enumerate(n_res_list):
        ...
        np.save(f"STDP_parallel_L{L}_N{ntag}_liq{k+1}_w_out.npy", W_dense)

    # PDF: histogram only
    pdf_name = f"STDP_parallel_L{L}_N{ntag}_wout_hist.pdf"
...
if __name__ == "__main__":
    for L in LIQUID_COUNTS:
        ...
        run_stdp_parallel_liquids(N_RES_BY_L[L])
