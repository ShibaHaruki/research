# ============================================================
# STDP_parallel_TEST_multi_liquid_L1to4_total1000.py
#
# 目的:
#  - STDP学習後に保存した重み（w_in, w_res, w_out）を読み込み
#  - 学習なし（固定重み）でテストデータを流し
#  - sout_rec (8, n_sample, N_out, 500) を保存
#
# 入力:
#  - sample_seq.npy
#  - tactile_data/<material>/data_<id>_*
#  - STDP_parallel_L{L}_N{ntag}_liq{k}_w_in.npy
#    STDP_parallel_L{L}_N{ntag}_liq{k}_w_res.npy
#    STDP_parallel_L{L}_N{ntag}_liq{k}_w_out.npy
#
# 出力:
#  - STDP_parallel_TEST_L{L}_N{ntag}_sout_rec.npy
#
# 追加:
#  - 進捗表示（全体バー、run中進捗(任意)）
...
BASE_PATH = str(script_path.parents[1]) + "/"   # ここを必要に応じて変更
...
# ============================================================
# 重みロード（学習コードの保存名に合わせる）
# ============================================================
def load_trained_weights(L: int, n_res_list: list[int]):
    ...
    for k, n_res in enumerate(n_res_list, start=1):
        f_in  = THIS_DIR / f"STDP_parallel_L{L}_N{ntag}_liq{k}_w_in.npy"
        f_res = THIS_DIR / f"STDP_parallel_L{L}_N{ntag}_liq{k}_w_res.npy"
        f_out = THIS_DIR / f"STDP_parallel_L{L}_N{ntag}_liq{k}_w_out.npy"
...
# テスト本体（並列リキッド）
# ============================================================
def run_test_parallel_liquids(L: int, n_res_list: list[int]):
    # 並列でも再現性を崩さない：Lごとにseedをずらしたローカルrng
    rng_local = np.random.default_rng(RNG_SEED + 1000 * L)

    ntag, N_in, N_out, W_in_list, W_res_list, W_out_list = load_trained_weights(L, n_res_list)
    ...
    for k, n_res in enumerate(n_res_list):
        ...
        # recurrent: connect only nonzero weights (exclude diagonal)
        W_res = W_res_list[k]
        ...
        # out: connect only nonzero weights（固定）
        W_out = W_out_list[k]
    ...
    t0 = 0 * ms  # サンプル開始時刻（apply_inputの基準）
    ...
    with tqdm(total=total_iters, desc=f"TEST L{L} N[{ntag}]", dynamic_ncols=True) as pbar:
    ...
    out_name = f"STDP_parallel_TEST_L{L}_N{ntag}_sout_rec.npy"
...
def _run_one_L(job):
    ...
    L, n_res_list = job
    run_test_parallel_liquids(L, n_res_list)
    return L
...
if __name__ == "__main__":
    ...
    if not PARALLEL_RUN:
        for L in tqdm(LIQUID_COUNTS, desc="L loop", dynamic_ncols=True):
            ...
            run_test_parallel_liquids(L, N_RES_BY_L[L])
