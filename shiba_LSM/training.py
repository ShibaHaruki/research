...
tau_m_res  = np.where(neuron_array_res == 1, 10*ms, np.where(neuron_array_res == -1, 10*ms, 0*ms))
...
tau_m_out = np.where(neuron_array_out == 1, 10*ms, np.where(neuron_array_out== -1, 10*ms, 0*ms))
...
G_res.tau_m = tau_m_res*ms;G_res.t_ref = t_ref_res*ms
G_out.tau_m = tau_m_out*ms;G_out.t_ref = t_ref_out*ms
