...
追加:
    ...
    - 線形分離特性 SP_lin(t) = rank(M_s(t)) の時間推移
      ※ 縦軸は正規化値ではなくランクそのもの
      ※ Al-Al は Al の15サンプルのみで評価
      ※ Al-Target は Al 15サンプル + Target 15サンプルで評価
...
# まとめ距離グラフ
# =========================================
def save_summary_distance_plot(
    *,
    time_ms: np.ndarray,
    states_base: np.ndarray,
    states_targets: dict[str, np.ndarray],
    ...
    for name, dists in all_dists.items():
        ...
        overall_time_mean = float(np.mean(dists))

        if name == f"{MAT_BASE}-{MAT_BASE}":
            ax.plot(time_ms, mean_dist, linewidth=2.5, label=f"{name} mean, {dists.shape[0]} pairs")
        else:
            ax.plot(
                time_ms,
                mean_dist,
                linewidth=2.5,
                linestyle="--",
                label=f"{name} mean, {dists.shape[0]} pairs",
            )

        ax.fill_between(time_ms, mean_dist - std_dist, mean_dist + std_dist, alpha=0.12)

        ax.axhline(
            overall_time_mean,
            linewidth=1.8,
            linestyle=":",
            label=f"{name} time mean = {overall_time_mean:.5f}",
        )
    ...
    if DISTANCE_MODE == "rms":
        ax.set_ylabel("RMS distance of liquid state x(t)", fontsize=14)
    else:
        ax.set_ylabel("Euclidean distance of liquid state x(t)", fontsize=14)

    ax.set_title(
        "Summary liquid-state distance\n"
        f"x_i(t) = (h * s_i)(t), tau_s={TAU_STATE_MS} ms",
        fontsize=15,
    )
    ...
    png_path = SUMMARY_DIR / f"liquid_state_distance_summary_multi_targets_rep{REP}.png"
...
# 線形分離特性 SP_lin
# =========================================
def calc_splin_over_time_single_material(
    states: np.ndarray,
    *,
    center_each_time: bool = True,
) -> tuple[np.ndarray, np.ndarray]:
    """
    1素材の15サンプルだけで SP_lin(t)=rank(M_s(t)) を計算する。

    states:
        shape = (n_samples, n_neurons, n_time)

    例:
        states shape = (15, 500, time)

    center_each_time=True の場合:
        最大ランク = min(n_samples - 1, n_neurons)
        N_COMPARE=15なら最大ランク14
    """

    if states.ndim != 3:
        raise ValueError("states must be 3D array: (samples, neurons, time)")
    ...
    if center_each_time:
        max_rank = min(n_samples - 1, n_neurons)
    else:
        max_rank = min(n_samples, n_neurons)
    ...
    return sp_rank, sp_norm


def calc_splin_over_time_for_pair(
    states_a: np.ndarray,
    states_b: np.ndarray,
    *,
    center_each_time: bool = True,
    ...
    """
    2素材間の SP_lin(t)=rank(M_s(t)) を計算する。

    states_a, states_b:
        shape = (n_samples, n_neurons, n_time)

    Al-Target の場合:
        Al 15サンプル + Target 15サンプル = 30サンプル

    center_each_time=True の場合:
        最大ランク = min(n_samples_total - 1, n_neurons)
        N_COMPARE=15なら最大ランク29
    """
    ...
    n_samples_total = states_a.shape[0] + states_b.shape[0]
    n_neurons = states_a.shape[1]

    if center_each_time:
        max_rank = min(n_samples_total - 1, n_neurons)
    else:
        max_rank = min(n_samples_total, n_neurons)
...
def save_pairwise_splin_time_transition_plot(
    *,
    ...
    # 同じ15サンプルを2回重ねない
    # -----------------------------
    sp_rank, sp_norm = calc_splin_over_time_single_material(
        states_base,
    ...
    pair_to_max_rank[base_pair_name] = min(states_base.shape[0] - 1, states_base.shape[1])
    ...
    for target_material, states_target in states_targets.items():
        ...
        sp_rank, sp_norm = calc_splin_over_time_for_pair(
            states_base,
            states_target,
            center_each_time=True,
        )
        ...
        pair_to_max_rank[pair_name] = min(
            states_base.shape[0] + states_target.shape[0] - 1,
            states_base.shape[1],
        )
    ...
    for pair_name, sp_rank in pair_to_splin_rank.items():
        ...
        if pair_name == base_pair_name:
            label = f"{pair_name}, n=15, max rank={max_rank}"
            ax.plot(
                time_ms,
                sp_rank,
                linewidth=2.5,
            ...
            label = f"{pair_name}, n=15+15, max rank={max_rank}"
            ax.plot(
                time_ms,
                sp_rank,
                linewidth=2.5,
    ...
    pdf_path = SUMMARY_DIR / f"splin_rank_time_transition_multi_targets_rep{REP}.pdf"
    ...
    csv_path = SUMMARY_DIR / f"splin_rank_time_transition_multi_targets_rep{REP}.csv"
...
def save_sample_mean_state_distribution_plot(
    *,
    ...
    n_bins = min(15, max(5, N_COMPARE * len(material_to_values)))
...
# =========================================
# Al-Al と Al-Target の距離平均ヒストグラム
# =========================================
def save_pair_distance_mean_histograms_by_target(
    *,
    ...
    if base_pair_name not in all_dists:
        raise KeyError(f"{base_pair_name} not found in all_dists")

    base_values_all = np.mean(all_dists[base_pair_name], axis=1)

    pair_name_to_values = {}
    summary_rows = []

    for target_material in TARGET_MATERIALS:
        target_pair_name = f"{MAT_BASE}-{target_material}"

        if target_pair_name not in all_dists:
            print(f"[skip] {target_pair_name} not found in all_dists")
            continue

        target_values_all = np.mean(all_dists[target_pair_name], axis=1)

        n_available = min(len(base_values_all), len(target_values_all))
        n_use = min(HIST_N_PAIRS, n_available)

        if n_use < HIST_N_PAIRS:
            print(
                f"[warning] requested HIST_N_PAIRS={HIST_N_PAIRS}, "
                f"but only n_use={n_use} available for {target_pair_name}"
            )

        base_idx = rng.choice(len(base_values_all), size=n_use, replace=False)
        target_idx = rng.choice(len(target_values_all), size=n_use, replace=False)

        base_values = base_values_all[base_idx]
        target_values = target_values_all[target_idx]

        pair_name_to_values[f"{base_pair_name}_used_for_{target_material}"] = base_values
        pair_name_to_values[target_pair_name] = target_values

        all_values = np.concatenate([base_values, target_values])
        ...
        ax.hist(
            ...
            edgecolor="tab:blue",
            linewidth=1.0,
            label=f"{base_pair_name} (n={n_use})",
        )
        ...
        ax.hist(
            ...
            linewidth=1.0,
            label=f"{target_pair_name} (n={n_use})",
        )
        ...
        ax.axvline(
            ...
            linestyle="--",
            linewidth=2.0,
            label=f"{base_pair_name} mean = {base_mean:.5f}",
        )
        ...
        ax.axvline(
            ...
            linestyle="--",
            linewidth=2.0,
            label=f"{target_pair_name} mean = {target_mean:.5f}",
        )

        ax.set_title(
            "Histogram of time-averaged pairwise liquid-state distances\n"
            f"{base_pair_name} vs {target_pair_name}",
            fontsize=15,
        )

        ax.set_xlabel(
            r"Time-averaged pairwise distance  $D_{ij}=\frac{1}{T}\sum_t d_{ij}(t)$",
            fontsize=13,
        )
        ax.set_ylabel("Count", fontsize=14)
        ...
        png_path = SUMMARY_DIR / (
            f"pair_mean_distance_histogram_"
            f"{MAT_BASE}_vs_{target_material}_rep{REP}.png"
        )
        fig.savefig(png_path, bbox_inches="tight", dpi=SAVE_DPI)
        plt.close(fig)
        ...
        for pair_name, values in [
            (base_pair_name, base_values),
            (target_pair_name, target_values),
        ]:
            summary_rows.append({
                "comparison_graph": f"{base_pair_name} vs {target_pair_name}",
                "pair_name": pair_name,
                "n_pairs_used": int(values.size),
                "mean": float(np.mean(values)),
                "std": float(np.std(values)),
                "min": float(np.min(values)),
                "max": float(np.max(values)),
    ...
    csv_path = SUMMARY_DIR / f"pair_mean_distance_histogram_by_target_summary_rep{REP}.csv"
    pd.DataFrame(summary_rows).to_csv(csv_path, index=False)
    print(f"[saved] {csv_path}")

    return pair_name_to_values
...
def main():
    ...
    print(f"HIST_N_PAIRS = {HIST_N_PAIRS}")
    ...
    base_base_rows = save_base_base_pair_distance_plots(
        time_ms=time_ms_ref,
        states_base=states_base,
        filter_base=filter_base,
    ...
    all_pair_rows.extend(base_base_rows)

    for target_material in TARGET_MATERIALS:
        rows = save_base_target_pair_distance_plots(
            target_material=target_material,
            time_ms=time_ms_ref,
            states_base=states_base,
            states_target=states_targets[target_material],
            filter_base=filter_base,
            filter_target=filters_targets[target_material],
            selected_seq=selected_seq,
            plot_limits=plot_limits,
        )
        all_pair_rows.extend(rows)

    pair_csv = SUMMARY_DIR / f"pair_distance_summary_multi_targets_rep{REP}.csv"
    ...
    # まとめ距離グラフ
    # -----------------------------
    all_dists = save_summary_distance_plot(
        time_ms=time_ms_ref,
        states_base=states_base,
        states_targets=states_targets,
        plot_limits=plot_limits,
    ...
    # -----------------------------
    # Al-Al vs Al-Target の距離平均値ヒストグラム
    # -----------------------------
    pair_mean_distance_values = save_pair_distance_mean_histograms_by_target(
        all_dists=all_dists,
        rng=rng,
    )
    ...
    for pair_name, values in pair_mean_distance_values.items():
        safe_pair_name = pair_name.replace("-", "_").replace("/", "_")
        save_payload[f"pair_mean_distance_values_{safe_pair_name}"] = values.astype(np.float32)
