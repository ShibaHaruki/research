...
def run_once_worker(prefix: str, rep: int, script_dir_str: str):
    ...
    # 学習則フォルダ内に置いた場合も、研究コード直下のデータを参照する。
    project_root = script_dir.parent if script_dir.name == "code" else script_dir.parents[1]
    ...
    # --- sample seq ---
    sample_path = script_dir / SAMPLE_SEQ_FMT.format(rep=rep)
    if not sample_path.exists():
        sample_path = project_root / "code" / SAMPLE_SEQ_FMT.format(rep=rep)
    sample_seq = np.load(sample_path).astype(int)
