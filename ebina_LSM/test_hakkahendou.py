...
class Synapse_model_out:
	...
	def __call__(self, s_record_res, s_record_out, batch=50):
...
def n_bunkatu_fireingrate(n_time, arr,dt):
	new_arr = np.array([])
	...
	for i in range(0, len(arr), n):
		...
		new_arr = np.append(new_arr, [sum(arr[i:i+n])])
...
#i_n_timeごとに発火率を求める(500だと分割なし)
i_n_time = 500  #ms
n_time = i_n_time * 1e-3 #s
nt_n_time = round(n_time/dt)



s_record_tortal = np.load("w_out_ltd0p051e-6_outneuron8_2input_bias-40_10s_record_tortal.npy")
base_size = 10
# print("w_out_ltd0p051e-6_outneuron8_2input_bias-40_10s_record_tortal")




s_record_tortal_n_time = np.zeros([N_out, round(nt/nt_n_time), base_size*sozai_n])
s_record_ave = np.zeros([sozai_n, round(nt/nt_n_time), N_out])

s_record_fr = np.zeros([sozai_n, N_out*round(nt/nt_n_time), base_size])
s_record_ave = np.zeros([sozai_n, N_out*round(nt/nt_n_time)])
s_record_cov = np.zeros([sozai_n, N_out*round(nt/nt_n_time), N_out*round(nt/nt_n_time)])


for i in range(sozai_n):
	for j in range(N_out):
		for k in range(base_size):
			s_record_tortal_n_time[j, :, i*base_size + k] = n_bunkatu_fireingrate(n_time, s_record_tortal[j, :, i*base_size + k], dt)

			s_record_fr[i, j*round(nt/nt_n_time):(j+1)*round(nt/nt_n_time), k] = s_record_tortal_n_time[j, :, i*base_size + k] / n_time
			s_record_ave[i, j*round(nt/nt_n_time):(j+1)*round(nt/nt_n_time)] += s_record_fr[i, j*round(nt/nt_n_time):(j+1)*round(nt/nt_n_time), k] /base_size 
...
w_out = np.load("w_out_ltd0p051e-6_outneuron8_2input_bias-40.npy")
...
loop_size = 10 	#loop_size回testを行い正答率のmax, ave, minを求める


loop_rec  = np.zeros(loop_size)
...
for loop_i in range(loop_size):
	...
	seigo = np.zeros([test_size*sozai_n, 2])
	...
	for i in range(len(dir_name)):	
		...
		test_data = rng.integers(100, 324, size=test_size)  #randomに残りのdataからtest_size個選択しtest


		#for i_size in range(test_size):
		for i_size in test_data:
			#print(i_size)
			df = pd.read_table(glob.glob("tactile_data/" +dir_name[i]+ f"/data_{i_size+1}_*")[0], header=None)
			df_np = df.to_numpy().T
			...
			for t in range(nt):
				...
				IPSC_out = synapse_out(s_record, s_record_out)
			...
			s_record_out_fr = np.zeros([N_out*round(nt/nt_n_time)])

			diff_sozai = np.zeros(sozai_n)
	

			#mahala
			for j in range(sozai_n):
				for k in range(N_out):

					s_record_out_fr[k*round(nt/nt_n_time) : (k+1)*round(nt/nt_n_time)] = n_bunkatu_fireingrate(n_time, s_record_out[k, :], dt) / n_time
				
				data_minus_ave = s_record_out_fr - s_record_ave[j,:]
				diff_sozai[j] = np.sqrt( (data_minus_ave.reshape([1,-1]) @ np.linalg.pinv(s_record_cov[j,:])) @ data_minus_ave.reshape([-1,1]))
		
				
			seigo[ii, 0] = i
			seigo[ii, 1] = np.argmin(diff_sozai)
	...
	for i in range(len(y_true)):
		if y_true[i] == y_pred[i]:
			count += 1
	count = count / (test_size*sozai_n)
	print(count)


	loop_rec[loop_i] = count
	if loop_i != 0:
