...
nt = 5000
base_size = 10
s_record_tortal = np.zeros([N_out, nt, base_size*sozai_n])
...
for i in range(len(dir_name)):	
	...
	s_record_sum_liq = np.zeros(N_liquid)
	s_record_sum_out = np.zeros(N_out)


	for i_size in range(base_size):
	#for i_size in rng.integers(100, size=base_size):
		...
		for t in tqdm(range(nt)):
			...
			# print(sum(sum(synapse_out.w_out)))
		...
		s_record_tortal[:,:,ii] = s_record_out

		ii += 1


		s_record_sum_liq += s_record.sum(axis=1) / T
		s_record_sum_out += s_record_out.sum(axis=1)/ T


np.save('w_out_ltd0p051e-6_outneuron8_2input_bias-40_10s_record_tortal', s_record_tortal)
