...
from f_run.run_test import TEST_RESULT_DIR, run_test, training_output_dir_for_cfg
from f_run.run_train_test_eval import run_train_test_eval
from f_run.run_train_test import run_train_test
from f_run.run_training import TRAINING_RESULT_DIR, build_cfg, build_network_cfg, run_training
...
def _result_root(target: str) -> Path:
    ...
    if target == "training":
        return TRAINING_RESULT_DIR
    if target == "liquid":
        return LIQUID_RESULT_DIR
    if target == "test":
        return TEST_RESULT_DIR
    if target == "train_test":
        return TRAINING_RESULT_DIR
    if target == "train_test_eval":
        return TRAINING_RESULT_DIR
    raise ValueError(f"Unknown target: {target}")


def _include_output(target: str) -> bool:
    return _normalize_target(target) != "liquid"
...
def _run_target(target: str, cfg: dict, rep: int) -> str:
    ...
    if target == "test":
        return run_test(rep, training_output_dir_for_cfg(cfg))
    if target == "train_test":
...
def _is_trial_level_target(target: str) -> bool:
    return _normalize_target(target) == "train_test_eval"
...
def _trial_result_payload(
    *,
    ...
    out_dir = make_run_output_dir(
        ...
        cfg,
        net_cfg,
        include_output=_include_output(target),
    )
    save_hierarchy_param_snapshots(
        out_dir,
        cfg,
    ...
    try:
        ...
        if not dry_run:
            if _is_trial_level_target(target):
                message = run_train_test_eval(cfg)
            elif _is_single_run_target(target):
...
def main() -> int:
    ...
    parser.add_argument(
        "--target",
        choices=("training", "liquid", "test", "train_test", "training_test", "train_test_eval", "training_test_eval"),
        default=None,
