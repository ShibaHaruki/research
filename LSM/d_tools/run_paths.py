...
def model_dir_name(model_cfg: dict, *, include_learning: bool = True) -> str:
    parts = [
        f"neuron_{safe_stem(model_cfg['NEURON_MODEL'])}",
        f"synapse_{safe_stem(model_cfg['SYNAPSE_MODEL'])}",
    ]
    if include_learning:
        parts.append(f"learn_{safe_stem(model_cfg['LEARNING_RULE_MODEL'])}")
...
def _liq_to_output_routes(net_cfg: dict, *, include_output: bool = True) -> dict[int, list[int]]:
    if not include_output:
        return {}

    p_pairs = net_cfg.get("p_liq_to_out_pairs")
    n_liq = len(_as_list(net_cfg.get("N_liq")))
    n_out = len(_as_list(net_cfg.get("N_out")))
    routes: dict[int, list[int]] = {}

    def add_route(out_index: int, liq_index: int, pair_prob) -> None:
        if _positive(pair_prob):
            routes.setdefault(out_index + 1, []).append(liq_index + 1)

    if isinstance(p_pairs, (list, tuple)):
        out_items = enumerate(p_pairs)
    elif isinstance(p_pairs, dict) and not _is_pair_config(p_pairs):
        out_items = ((int(out_idx), value) for out_idx, value in p_pairs.items())
    else:
        out_items = ((out_idx, p_pairs) for out_idx in range(n_out))

    for out_idx, out_value in out_items:
        if _is_pair_config(out_value):
            for liq_idx in range(n_liq):
                add_route(int(out_idx), liq_idx, out_value)
            continue

        if isinstance(out_value, dict):
            for liq_idx, pair_prob in out_value.items():
                add_route(int(out_idx), int(liq_idx), pair_prob)

    return {out_idx: sorted(set(liq_layers)) for out_idx, liq_layers in sorted(routes.items())}
...
def network_topology(net_cfg: dict, *, include_output: bool = True) -> dict:
    ...
    out_routes = _liq_to_output_routes(net_cfg, include_output=include_output)
    out_labels = [
        f"{_format_layers(liq_layers, 'L')}_to_O{out_idx}"
        for out_idx, liq_layers in out_routes.items()
    ]
    ...
    if include_output:
        topology["liquid_to_output"] = {
            "routes": {f"O{out_idx}": liq_layers for out_idx, liq_layers in out_routes.items()},
            "label": "liq2out_" + ("__".join(out_labels) if out_labels else "none"),
        }
    return topology
...
def network_dir_name(net_cfg: dict, include_output: bool = True) -> str:
    ...
    if include_output:
        key_cfg.update(
            {
                "N_out": net_cfg.get("N_out"),
                "r_inh_out": net_cfg.get("r_inh_out"),
                "p_liq_to_out_pairs": net_cfg.get("p_liq_to_out_pairs"),
                "gain_liq_to_out_pairs": net_cfg.get("gain_liq_to_out_pairs"),
            }
        )
    ...
    n_out = "-".join(value_stem(v) for v in net_cfg.get("N_out", []))
    out_label = topology["liquid_to_output"]["label"]
    return (
        f"Nliq_{n_liq}__Nout_{n_out}__{in_label}__{rec_label}__"
        f"{out_label}__{conn}__{digest}"
    )
...
def network_params_snapshot(cfg: dict, net_cfg: dict, *, include_output: bool = True) -> dict:
    ...
    selected_keys = keys + (output_keys if include_output else ())

    return {
        "network_dir": network_dir_name(net_cfg, include_output=include_output),
        "include_output": include_output,
        "topology": network_topology(net_cfg, include_output=include_output),
        "network": _pick_keys(net_cfg, selected_keys),
...
def model_params_snapshot(cfg: dict, *, include_learning: bool = True) -> dict:
    ...
    snapshot = {
        ...
        "models": _pick_keys(
            model_cfg,
            model_keys + (("LEARNING_RULE_MODEL",) if include_learning else ()),
        ),
    ...
    if include_learning:
        learning_name = model_cfg["LEARNING_RULE_MODEL"]
        snapshot["learning_rule_params"] = cfg.get("learning_rule_models", {}).get(
            learning_name,
            {},
        )
...
def config_snapshot_payload(
    ...
    *,
    ...
    if include_learning is None:
        include_learning = include_output
    model_keys = ("NEURON_MODEL", "SYNAPSE_MODEL") + (
        ("LEARNING_RULE_MODEL",) if include_learning else ()
    )
