...
BCPNN_RULE = register_model(
    LEARNING_RULES,
    "BCPNN",
    learning_rule(
        eqs="""
dZi/dt   = -Zi/tau_z             : 1 (clock-driven)
dZj/dt   = -Zj/tau_z             : 1 (clock-driven)
dPi/dt   = (Zi - Pi)/tau_p       : 1 (clock-driven)
dPj/dt   = (Zj - Pj)/tau_p       : 1 (clock-driven)
dPij/dt  = (Zi*Zj - Pij)/tau_p   : 1 (clock-driven)
""",
        on_pre="""
Zi += z_inc
w = clip(bcpnn_gain * log((Pij + bcpnn_eps) / (Pi * Pj + bcpnn_eps)) + bcpnn_w_offset, wmin, wmax)
""",
        on_post="""
Zj += z_inc
w = clip(bcpnn_gain * log((Pij + bcpnn_eps) / (Pi * Pj + bcpnn_eps)) + bcpnn_w_offset, wmin, wmax)
