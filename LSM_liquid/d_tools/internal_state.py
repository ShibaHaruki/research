...
def _monitor_spikes(
    ...
    *,
    ...
    if spike_t_ms.size:
        candidates = [spike_t_ms]
        if start_time is not None:
            candidates.append(spike_t_ms - np.float32(float(start_time / ms)))
        candidates.append(spike_t_ms - np.float32(float(np.min(spike_t_ms))))
...
def save_internal_states_from_snapshots(
    ...
    *,
    ...
    for snapshot in layer_snapshots:
        ...
        spike_indices = np.asarray(snapshot.get("spike_i", ()), dtype=np.int32)
        if spike_times.size and spike_indices.size:
            valid_spikes = (
                (spike_indices >= 0)
                & (spike_indices < n_neurons)
                & np.isfinite(spike_times)
            )
            arrays["spike_t_ms"] = spike_times[valid_spikes]
            arrays["spike_i"] = spike_indices[valid_spikes]
            all_spike_t.append(spike_times[valid_spikes])
            all_spike_i.append(spike_indices[valid_spikes] + np.int32(neuron_offset))
        else:
            arrays["spike_t_ms"] = np.asarray([], dtype=np.float32)
            arrays["spike_i"] = np.asarray([], dtype=np.int32)
    ...
    combined_payload = {
        ...
        "spike_t_ms": (
            np.concatenate(all_spike_t).astype(np.float32, copy=False)
            if all_spike_t
            else np.asarray([], dtype=np.float32)
        ),
        "spike_i": (
            np.concatenate(all_spike_i).astype(np.int32, copy=False)
            if all_spike_i
